...
(define iterate-result-data
  (lambda (result)
	...
	;; cdr through result - every top level list is a org
	(when (> (length result) 0)
	  (let ((child (cdr result)))
		(when (and (list? child)
				   (> (length child) 0))
		  (if (list? (car child))
			(extract-org-info (car child))))))))
...
(define data-port (open-file (string-append (getcwd) "/org-data.xml") "r"))
(define sxml-data (xml->sxml data-port))

;; code to save output file
;;(define output-port (open-file (string-append (getcwd) "/org-data.scm") "w"))
